...
?>
		...
		<p><strong>Request for closing sales order has been inputed</strong></p>
<?php
	if($close_sales_order->requested_by	== 1){
		$requested_by	= 'Corresponding sales';
	} else if($close_sales_order->requested_by == 2){
		$requested_by	= 'Customer';
	} else if($close_sales_order->requested_by == 3){
		$requested_by	= 'Other';
	}
?>
		<table class='table table-bordered'>
			...
			<tr>
				...
				<td><?= $close_sales_order->information ?></td>
			</tr>
		</table>
		
		<a href='<?= site_url('Sales_order/track') ?>'><button type='button' class='button button_default_dark'><i class='fa fa-long-arrow-left'></i></button></a>
	</div>
</div>
<script>
