<head>
	<title>Sales return - Cancel Dashboard</title>
</head>
<div class='dashboard'>
	<div class='dashboard_head'>
		<p style='font-family:museo'><a href='<?= site_url('Administrators') ?>' title='Administrator'><i class='fa fa-briefcase'></i></a> /Sales Return / Cancel</p>
	</div>
	<br>
...
<script>
	...
	function refresh_view(page = $('#page').val()){
		$.ajax({
			url:'<?= site_url('Sales_return/getConfirmedReturn') ?>',
			data:{
				page: page,
				month: $('#month').val(),
				year: $('#year').val()
	...
	function viewSalesReturn(n){
		$.ajax({
			...
			success:function(response){
				salesReturnId	= n;
				var general		= response.salesReturn;
				var name		= general.documentName;
				...
				$.each(items, function(index, item){
					...
					$('#salesReturnItemTable').append("<tr><td>" + reference + "</td><td>" + name + "</td><td>" + numeral(quantity).format('0,0') + "</td><td>" + numeral(received).format('0,0') + "</td></tr>");
				});
	...
	function deleteSalesReturn(){
		$.ajax({
			url:"<?= site_url('Sales_return/closeSalesReturnById') ?>",
			data:{
				id: salesReturnId
			},
			type:"POST",
