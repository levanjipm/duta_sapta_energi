...
<div class='dashboard'>
	<div class='dashboard_head'>
		<p style='font-family:museo'><a href='<?= site_url('Administrators') ?>' title='Administrator'><i class='fa fa-briefcase'></i></a> /Purchase Order / Close</p>
	</div>
	<br>
			...
			success:function(response){
				...
				$.each(items, function(index, item){
					...
					if(is_confirm == 1 && is_delete == 0){
						$('#purchaseOrderTableContent').append("<tr><td><label>" + name + "</label><p>" + my_date_format(date) + "</p></td><td><label>" + supplierName + "</label><p>" + complete_address + "</p><p>" + supplierCity + "</p></td><td><button class='button button_danger_dark' onclick='viewPurchaseOrder(" + id + ")' title='Close " + name + "'><i class='fa fa-trash'></i></button>")
						countPurchaseOrder++;
					}
				})
