<head>
	<title>Purchase return - Cancel Dashboard</title>
</head>
<div class='dashboard'>
...
<script>
	...
	function refresh_view(page = $('#page').val()){
		$.ajax({
			url:'<?= site_url('Purchase_return/getIncompletedPurchaseReturn') ?>',
			data:{
				page: page,
			},
			type:'GET',
			success:function(response){
				...
				$.each(items, function(index, item){
					...
					var id					= item.id;
					var name				= item.name;
					$('#purchaseReturnTableContent').append("<tr><td>" + my_date_format(date) + "</td><td>" + name + "</td><td>" + supplierName + "</td><td><button class='button button_default_dark' onclick='viewPurchaseReturn(" + id + ")'><i class='fa fa-eye'></i></button></td></tr>");
					countSalesReturn++;
				});
		...
		$.ajax({
			...
			success:function(response){
				...
				$.each(items, function(index, item){
					...
					$('#purchaseReturnItemTable').append("<tr><td>" + reference + "</td><td>" + name + "</td><td>" + numeral(quantity).format('0,0') + "</td><td>" + numeral(sent).format('0,0') + "</td></tr>");
				});
	...
	function deletePurchaseReturn(){
		$.ajax({
			url:"<?= site_url('Purchase_return/cancelById') ?>",
			data:{
				id: purchaseReturnId
			},
			type:"POST",
