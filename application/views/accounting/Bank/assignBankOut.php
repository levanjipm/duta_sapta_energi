...
?>
				<tr>
					...
					<td>
						Rp. <span id='remainingValue-<?= $id ?>'><?= number_format($value - $paid,2) ?></span>
...
<?php } else { ?>
				<tr>
					<td><?= date('d M Y',strtotime($date)) ?></td>
					<td><?= $name ?></td>
					<td><input type='checkbox' name='blankCheckBox[<?= $id ?>]' onchange='blankProcess(<?= $id ?>)' id='blankCheckBox-<?= $id ?>'></td>
					<td>
						Rp. <span id='blankRemainingValue-<?= $id ?>'><?= number_format($value - $paid,2) ?></span>
						<input type='hidden' id='blankRemaining-<?= $id ?>' value='<?= $value - $paid ?>' name='blankRemaining[<?= $id ?>]'>
						<input type='hidden' id='blankOriginal-<?= $id ?>' value='<?= $value - $paid ?>' name='blankOriginal[<?= $id ?>]'>
					</td>
				</tr>
<?php } } ?>
			</table>
...
<script>
	function process(id){
		...
		var len = $("[name*='checkBox']:checked").length + $("[name*='blankCheckBox']:checked").length;
		if(len > 0){
			$('#submit_button').show();
	...
	function blankProcess(id){
		if($('#blankCheckBox-' + id).prop('checked') == true){
			var remaining_bank_value	= parseFloat($('#bank_value').val());
			var remaining_invoice_value	= parseFloat($('#blankRemaining-' + id).val());
			var difference				= remaining_bank_value - remaining_invoice_value;
			if(remaining_bank_value > remaining_invoice_value){
				$('#bank_value').val(Math.abs(difference));
				$('#blankRemaining-' + id).val(0);
				$('#bank_value_p').html(numeral(Math.abs(difference)).format('0,0.00'));
				$('#blankRemainingValue-' + id).html(numeral(0).format('0,0.00'));
			} else {
				$('#bank_value').val(0);
				$('#blankRemaining-' + id).val(Math.abs(difference));
				$('#bank_value_p').html(numeral(0).format('0,0.00'));
				$('#blankRemainingValue-' + id).html(numeral(Math.abs(difference)).format('0,0.00'));
			}
		} else {
			var original_value		= parseFloat($('#blankOriginal-' + id).val());
			var current_bank_value	= parseFloat($('#bank_value').val());
			var current_value		= parseFloat($('#blankRemaining-' + id).val());
			var difference			= parseFloat(Math.abs(original_value - current_value));
			
			var bank_value			= current_bank_value + difference;
			
			$('#blankRemaining-' + id).val(original_value);
			$('#blankRemainingValue-' + id).html(numeral(original_value).format('0,0.00'));
