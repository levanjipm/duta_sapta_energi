...
class Stock_in_model extends CI_Model {
	private $table_stock_in = 'stock_in';
		...
		{
			$query		= $this->db->query("
				SELECT item.reference, item.name, item.id, (COALESCE(current.quantity, 0) - COALESCE(processOut.quantity, 0) + COALESCE(processIn.quantity, 0)) AS stock, processOut.quantity AS processOut, processIn.quantity AS processIn, COALESCE(processPurchase.quantity, 0) AS processPurchase
				FROM item
				LEFT JOIN (
				...
				LEFT JOIN (
					SELECT SUM(purchase_order.quantity - received) AS quantity, purchase_order.item_id
					FROM purchase_order
					WHERE purchase_order.status = 0
					GROUP BY purchase_order.item_id
				) processPurchase
				ON processPurchase.item_id = item.id
		...
		public function ViewCard($item_id, $offset = 0)
		{
			$query = $this->db->query("
				SELECT * FROM (
				...
				ORDER BY cardTable.date DESC, cardTable.documentId ASC, cardTable.quantity DESC, documentType ASC
		...
		public function ViewPreviousStock($item_id, $offset){
			...
			$query		= $this->db->query("
				...
				(
					...
					WHERE cardTable.documentId IS NOT NULL
					ORDER BY cardTable.date DESC, cardTable.documentId ASC, cardTable.quantity DESC, documentType ASC
					LIMIT 18446744073709551615 OFFSET $adjustedOffset
				) AS a
			");
		...
		public function countCard($item_id)
		{
			$query			= $this->db->query("
				SELECT * FROM (
					SELECT COALESCE(customer.name, supplier.name, 'Internal Transaction') as name, COALESCE(deliveryOrderTable.date, eventTable.date,purchaseReturnTable.date) as date, COALESCE(deliveryOrderTable.quantity, eventTable.quantity, purchaseReturnTable.quantity) * (-1) as quantity, COALESCE(deliveryOrderTable.name, eventTable.name, purchaseReturnTable.name) as documentName, COALESCE(deliveryOrderTable.deliveryOrderId, eventTable.eventId, purchaseReturnTable.returnId) AS documentId, COALESCE(deliveryOrderTable.type, eventTable.type, purchaseReturnTable.type) AS documentType
