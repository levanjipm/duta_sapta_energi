...
class Sales_return_received_detail_model extends CI_Model {
	private $table_sales_return = 'sales_return_received';
		...
		{
			$query			= $this->db->query("
				SELECT DISTINCT(sales_return_received.id) as id, item.id as item_id, item.reference, item.name, sales_return_received.quantity, sales_return.price, sales_return.id AS salesReturnId, sales_return.received, SUM(sales_return_received.quantity * sales_return.price) AS value
